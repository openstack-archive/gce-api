...
class API(base_api.API):
    ...
    def add_item(self, context, name, body, scope=None):
        ...
        context.operation_data["diskDevice"] = 0
    ...
    def _create_instance(self, context):
        ...
        diskDevice = context.operation_data["diskDevice"]
        instance = context.operation_data.get("instance")
        progress = {"progress": int(100.0 * diskDevice / full_count)}
        ...
        if disk:
            ...
            context.operation_data.pop("disk")
            diskDevice += 1
            context.operation_data["diskDevice"] = diskDevice
            progress["progress"] = int(100.0 * diskDevice / full_count)
        ...
        while diskDevice < len(disks):
            disk = disks[diskDevice]
            if "initializeParams" in disk:
                ida = instance_disk_api.API()
                disk["id"] = ida.create(context, args[0],
                                        disk["initializeParams"], scope)
                context.operation_data["disk"] = disk
    ...
    def _get_add_item_progress(self, context, instance_id):
        ...
        if instance.status != "BUILD":
            return operation_util.gef_final_progress(instance.status
                                                     == "ERROR")
        return None
