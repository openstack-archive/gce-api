...
"""gceapi HACKING file compliance testing
...
IMPORT_EXCEPTIONS = ['sqlalchemy', 'migrate', 'gceapi.db.sqlalchemy.session',
...
def gceapi_todo_format(physical_line, tokens):
    """Check for 'TODO()'.

    gceapi HACKING guide recommendation for TODO:
...
def gceapi_except_format(logical_line):
    r"""Check for 'except:'.

    gceapi HACKING guide recommends not using except:
...
def gceapi_except_format_assert(logical_line):
    r"""Check for 'assertRaises(Exception'.

    gceapi HACKING guide recommends not using assertRaises(Exception...):
    Do not use overly broad Exception type

    Okay: self.assertRaises(gceapiException)
...
def gceapi_import_rules(logical_line):
    r"""Check for imports.

    gceapi HACKING guide recommends one import per line:
    Do not import more than one module per line

    Examples:
    Okay: from gceapi.compute import api
    N301: from gceapi.compute import api, utils


    Imports should usually be on separate lines.

    gceapi HACKING guide recommends importing only modules:
    ...
    Okay: from os import (path as p)
    Okay: import os.path
    Okay: from gceapi.compute import rpcapi
...
def gceapi_import_alphabetical(logical_line, blank_lines, previous_logical,
                             indent_level, previous_indent_level):
    r"""Check for imports in alphabetical order.

    gceapi HACKING guide recommendation for imports:
...
def gceapi_import_no_db_in_virt(logical_line, filename):
    """Check for db calls from gceapi/virt

    As of grizzly-2 all the database calls have been removed from
    gceapi/virt, and we want to keep it that way.

    N307
    """
    if "gceapi/virt" in filename and not filename.endswith("fake.py"):
        if logical_line.startswith("from gceapi import db"):
            yield (0, "N307: gceapi.db import not allowed in gceapi/virt/*")
...
def gceapi_docstring_start_space(physical_line, previous_logical):
    r"""Check for docstring not start with space.

    gceapi HACKING guide recommendation for docstring:
...
def gceapi_docstring_one_line(physical_line, previous_logical):
    r"""Check one line docstring end.

    gceapi HACKING guide recommendation for one line docstring:
...
def gceapi_docstring_multiline_end(physical_line, previous_logical, tokens):
...
def gceapi_docstring_multiline_start(physical_line, previous_logical, tokens):
...
def gceapi_no_cr(physical_line):
...
def gceapi_localization_strings(logical_line, tokens):
...
def gceapi_is_not(logical_line):
    r"""Check localization in line.
...
def gceapi_not_in(logical_line):
    r"""Check localization in line.
...
def add_gceapi():
    """Monkey patch in gceapi guidelines.

    Look for functions that start with gceapi_  and have arguments
    ...
    for name, function in globals().items():
        ...
        if args and name.startswith("gceapi"):
...
def once_git_check_commit_title():
    ...
    gceapi HACKING recommends not referencing a bug or blueprint in first line,
...
if __name__ == "__main__":
    ...
    #gceapi error codes start with an N
